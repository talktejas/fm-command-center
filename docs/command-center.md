...
The list opens on the newest 200 and `Show older messages` walks back through the rest a window at a time, saying how many of the log's messages are loaded, and not shipping all of it at once is not the same as dropping any of it. Search is answered from the whole log, however far back a match is, not only from what is on screen, and a message is found by your own replies to it as well as by its own words. A message you have Held is on the page (held is one more thing a row can be, not a second copy of it) but is never in Messages, Archived or Info - it is counted, and shown, only under **On hold** - so the Messages and Info badges are exactly what their own tab lists, counted from what is loaded, never a total that includes rows those two tabs do not show.

**Info** is the same log, split off from Messages (his report 2026-09-24: a badge of 28 over an empty list, held messages counted where they were never shown; and separately, "put the routine stuff in an info tab"): pure progress and no-change chatter - "nothing new", "still running", a stopped-worker ack and the like - with nothing else about it. Anything reporting a real outcome, a landed change, a decision or a problem stays in Messages, and the split is deliberately conservative: a message it cannot confidently call info-only stays in Messages rather than risk hiding something that needs you, and a message the recorder marked, or that plainly reads, as a question is never info-only regardless of what else it says. Grouping, search, Archive and Hold all work the same as Messages, because it is the same rows, filtered.
...
**Waiting on you** is the queue firstmate is still holding, from three kinds of durable record:
...
- A **message that needs a reply**: recorded as a question, or plainly asking to be one (a literal question, "reply 1 or 2", "say the word", "your call", "waiting on your", a `merge ...?`, "should I ...?", "reply \"...\"", "tell me", "let me know") - his report 2026-09-24: dozens of messages plainly ask him something without ever being flagged as a question by the recorder. Answering one settles it - the message leaves Waiting on you but stays in Messages, the same as a settled captain hold stays in the backlog - unless your reply only asks firstmate something back (a literal question, "what do you mean", "explain", "more info", "which one", "why"): that never counts as the decision it was waiting on, so the message stays listed, still flagged, until a reply that actually decides something lands.
...
**Threading**: his report 2026-09-24: "i rec'd no answer for this yet" - he replied to a message and firstmate's actual answer landed as a brand-new row in Messages, with no way to see it belonged under his reply. Every conversation pane (a message, or a Waiting-on-you item) now shows the whole thread in order - firstmate's original, his reply, then, when the rule below finds one, firstmate's answer, then his next reply, and so on - and carries a marker while it is open: **answered** once one lands after his latest reply, **awaiting firstmate** with the wait time otherwise. The matching rule (`threadRows`/`matchAnswer`, `web/command-center-state.js`), tried in order and never guessed past: (1) a later message recorded against the SAME task, sent after this reply and before his next one in the same conversation; (2) failing that, a later message that plainly quotes or names the same subject; (3) otherwise the reply stays unanswered here - a wrong thread is worse than none. My words opens the same pane, so a reply threads there too.
...
**My words** is everything you have typed here and where it went. His report 2026-09-24: a row used to show only his own words, with no way to tell what they were replying to. Every row now shows what it answered above his own words: the firstmate message or Waiting-on-you item's title, enough of its own text to recognise it, and its project/worktree/branch - a click still opens that original in full. A genuinely standalone note (nothing it replied to) says so plainly instead. The opened pane on the right shows the same thing, in the same message-bubble style as the Messages tab: the original first, his reply underneath it.
...
Above the list, every notice that applies is shown as its own band, because two independent facts never share one slot and none of them pushes another off the screen: whatever has gone wrong between the page and the records, a backlog whose holds are hidden, and whether the MAIN home is watched, with its last beat from `state/.last-watcher-beat`. Only the main home's watch state is shown: every answer now goes to its inbox regardless of which home an item belongs to (see **Where your answer goes**), so a second mate's own watcher state is no longer a fact that changes what you can do here.
If the main home has gone quiet, an answer you send is still recorded but nothing will ring it, and the page says so rather than looking normal.
...
Every answer in **Waiting on you** goes to the MAIN home's captain inbox, through `bin/fm-inbox.sh note` with `FM_HOME` set to the main home — the exact path a plain note already takes — and nowhere else, whatever home the item itself belongs to. An idle second mate (b2b, INTERACT, ...) keeps no watcher running, so a note left in its own inbox would wait unread; the main home is always watched, so every answer routes there instead. Nothing on this send path calls `bin/fm-captain-hold.sh` or `bin/fm-send.sh`: both own a real decision record, but both are bounded by work a click must not wait on (a remote ledger read, a worker's own steering inbox). Firstmate reads the note and closes the decision itself, on its own next turn, whatever kind of row it was, forwarding it on with `fm-send` when the note names another home.

There are exactly two outcomes, because there is exactly one route: **sent** (the note is durably queued) or **failed** (it never made it, so your text stays in the box and sending again is safe — a second note is at worst a duplicate, never a duplicate steer). The note is prefixed with the item's own home and id (e.g. `[b2b · kk-xyz]`) ahead of your words, so firstmate can tell which decision it resolves and which home to forward it to.

Replying never removes a captain-hold or open-status item from **Waiting on you** on its own — only your own explicit Archive click does that (see **Archive**), because a reply is sometimes just a comment you want to keep checking on. A message that needed a reply is different: a reply that decides something does leave it, the same way it always left Messages, and never archives the message it answers on its own (see the previous paragraph and **Archive**).
