...
Firstmate's own capture and backfill stop there by design, so this page goes one step further on every read: whatever they still left blank is filled here, from the current records, and never written back to firstmate's log. A row naming a task whose `state/<id>.meta` is gone still gets its project from the task's backlog line (`repo:`), though never a worktree or a branch it no longer has. A row naming no task at all is matched against every task id and every registered project (`data/projects.md`) the words of its own title and text contain; a match is used only when it is the one and only match, and a row that matches nothing, or matches more than one, is left exactly as blank as firstmate left it. Wherever this page filled in more than firstmate itself recorded, a small quiet line under Project/Worktree/Branch names where it came from - a task id it was recorded against, a task id it matched in the text, or a project name it matched.
